...
def main():
   ...
   os.system('mpc clear')
   os.system('mpc load lista')
   cmd = subprocess.Popen("mpc playlist",shell=True, stdout=subprocess.PIPE)
   stations = cmd.stdout.readlines()

   lcd_ascii168_string(0,4,"Raspberry Radio")
   lcd_ascii168_string(0,6,"Stacji: " + str(len(stations)))
      ...
      if(prev==True):
         if(GPIO.input(PREV)==False):
            if(currentChannel<channelCount):
               ...
               currentChannel = 1
            cmd = subprocess.Popen("mpc play " + str(currentChannel),shell=True, stdout=subprocess.PIPE)
            led(True)
            title = cmd.stdout.readline()
            print str(currentChannel) + ": " + title
            lcd_ascii168_string(0,0,str(title))
      next = GPIO.input(PREV)
...
def lcd_init():
   ...
   lcd_tranfer_data(0x23,0);    #V5 voltage regulator internal resistor ratio set(contrast)
               ##20~27 small~large

   lcd_tranfer_data(0x81,0);    #Electronic volume mode set
               ##81: Set the V5 output voltage

   lcd_tranfer_data(0x27,0);    #Electronic volume register set
